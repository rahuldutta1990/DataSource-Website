      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-500">No articles matched your criteria.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filtered.map((post) => (
                <article
                  key={post.id}
                  className="consulting-card bg-white rounded-2xl border border-slate-200/90 overflow-hidden flex flex-col justify-between shadow-sm"
                >
                  <div>
                    ...
                    <div className="p-6">
                      <div className="flex items-center gap-3 text-xs text-slate-400 mb-3">
                        <span className="font-bold text-[#0077FF] uppercase tracking-wider">
                          {post.category}
                        </span>
                        <span>•</span>
                      ...
                      <h2 className="text-lg font-bold text-slate-900 font-heading leading-snug hover:text-[#0077FF] transition-colors mb-3">
                        <Link to={`/insights/${post.slug}`}>{post.title}</Link>
                      </h2>
                      <p className="text-sm text-slate-600 line-clamp-3 leading-relaxed font-body">
                        {post.excerpt}
                      </p>
                    </div>
                  </div>
